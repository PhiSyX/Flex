	...
	ApplicationAdapter: ApplicationAdapterInterface,
{
	...
	) -> Self
	{
		...
		let logger_signal =
			LoggerSignal::create(&application_name, &application_version);
...
impl<A, E, C> Kernel<A, E, C>
{
	...
	pub fn define_project_directory(
		mut self,
	...
	pub fn define_config_directory(
		mut self,
		dir: impl Into<path::PathBuf>,
	) -> Self
	where
	...
	pub fn define_loader_extension(
		mut self,
		loader_extension: impl Into<flex_fs::Extension>,
	) -> Self
	{
	...
	O: serde::de::DeserializeOwned,
{
	...
	if let Some(config_directory) = settings.directory.config() {
		flex_fs::load(config_directory, filepath, settings.loader_extension)
	} else {
	...
	O: serde::ser::Serialize + serde::de::DeserializeOwned,
{
	...
	if let Some(config_directory) = settings.directory.config() {
		flex_fs::load_or_prompt(
			config_directory,
			filepath,
			settings.loader_extension,
		)
	} else {
		Err(std::io::Error::new(
			std::io::ErrorKind::NotFound,
