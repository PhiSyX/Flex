...
impl<A, E, C> ApplicationLoggerExtension for Kernel<A, E, C>
{
			...
			| Err(err) => {
				self.logger_signal.send_warning(format!(
					"Le fichier de configuration du logger n'a pas pu être \
					 chargé correctement.\nRaison « {err} ». \nLes paramètres \
					 par défaut du logger ont été appliqués.",
				));
	...
	fn with_logger(self, settings: impl Into<LoggerSettings>) -> Self
	{
		...
		let level_based_on_process_mode = match self.settings.process_mode {
			| ProcessMode::LOCAL => flex_logger::LevelFilter::Debug,
			| ProcessMode::DEVELOPMENT => flex_logger::LevelFilter::Debug,
			| ProcessMode::PRODUCTION => flex_logger::LevelFilter::Error,
			| ProcessMode::TEST => flex_logger::LevelFilter::Trace,
		};
