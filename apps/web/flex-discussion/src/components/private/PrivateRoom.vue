...
let maybe_recipient = computed(() => view.maybe_recipient);
...
<template>
	<Match :maybe="maybe_recipient">
		<template #some="{ data: c_recipient }">
			...
			>
				<template #avatar="{ recipient }">
					<Avatar
						...
						@click="
							view.show_private_menu_handler($event, recipient)
						"
					/>
				</template>
			</PrivateRoomComponent>
